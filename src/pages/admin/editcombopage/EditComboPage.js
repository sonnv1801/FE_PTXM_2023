...
const EditComboPage = () => {
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_API_URL}/v1/combo/${path}`)
      .then((res) => res.json())
  ...
  const handleSaveChanges = () => {
    ...
    const url = `${process.env.REACT_APP_API_URL}/v1/combo/combos/${path}`;
  ...
  const handleAddProductToCombo = async () => {
    try {
      ...
      const url = `${process.env.REACT_APP_API_URL}/v1/combo/combo/${path}/products`;
  ...
  const handleRemoveProduct = (index) => {
    ...
    axios
      .delete(
        `${process.env.REACT_APP_API_URL}/v1/combo/combo/${path}/product/${productId}`
      )
      .then(() => {
        setData((prevData) => {
          const updatedProducts = [...prevData.products];
